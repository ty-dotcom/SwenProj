...
export async function unregisterPushSubscriptionToServer() {
    const existingSubscription = await getCurrentPushSubscription();
...
export async function sendPushSubscriptionToServer(subscription: PushSubscription) {
    ...
    if (!response.ok) {
        throw new Error("Failed to push subscription to server.");
    }
}
...
export async function deletePushSubscriptionFromServer(subscription: PushSubscription) {
    ...
    if (!response.ok) {
        throw new Error("Failed to delete push subscription to server.");
    }
